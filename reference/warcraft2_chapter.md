...
在③处，武器的种类由 `nNo % WEAPON_NUM` 决定——编号为 1 的 dragon 拿 sword（0），编号为 2 的拿 bomb（1），编号为 3 的拿 arrow（2），如此循环。在④处，士气值等于司令部**生产完这名 dragon 之后**的剩余生命元，除以 dragon 的初始生命值，用浮点数存储。注意必须强制转换为 `double` 再做除法，否则整除会丢失小数部分。
