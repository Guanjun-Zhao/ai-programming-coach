...
## Cursor / VS Code（类型检查与 PyQt6）

仓库已包含 `.vscode/settings.json`，默认使用项目内虚拟环境：

先执行：`python -m venv .venv`，再 `pip install -r requirements.txt`。

解释器路径：`${workspaceFolder}/.venv/Scripts/python.exe`。打开本项目后若 Problems 仍提示找不到 `PyQt6`，在命令面板执行 **Python: Select Interpreter**，选择 `.venv` 下的 Python，或 **Developer: Reload Window**。
...
- `docs/PYQT_TEMPLATE_HANDOFF.md`：**PyQt 界面模板交接**（师圣晴）
